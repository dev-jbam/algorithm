...
long long int dist[MAX]; // 가장 가까운 어떤 대피소 까지의 거리를 저장한다. 
int n, m, k;
...
int dijkstra(vector< vector< pair< int, int > > > &graph, priority_queue < node, vector<node>, greater<node> >  &pq){
	...
	while (pq.empty() == false){
		...
		long long int weight = pq.top().weight;
		int cur = pq.top().to;
		pq.pop();
		...
		for (int i = 0; i < graph[cur].size(); i++){
			...
			long long int nweight = graph[cur][i].second + dist[cur];

			if (dist[next] >  nweight){
				dist[next] = nweight;
				visit[next] = visit[cur];
				node tmp(dist[next], next, visit[cur]);
				pq.push(tmp);
			}

			else if (dist[next] == nweight ){
